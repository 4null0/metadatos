...
def printMeta(opcion, target, color_mode): 
	...
	if opcion == "f":
		...
		if ext == "pdf":
			print_pdf(target, color_mode)
		elif ext == "docx" or ext == "xlsx":
			print_docx(target, color_mode)
		elif ext in ("jpg","jpeg",): 
			print_jpg(target, color_mode)
		elif ext in ("png","gif","bmp"): 
			print_png_gif_bmp(target, color_mode)
		elif ext in ("tif","tiff"):
			print_tiff(target, color_mode)
		else: 
			pass
...
def print_docx(file_full_path, color_mode): 
	...
	zf = zipfile.ZipFile(file_full_path, "r")
		...
		if ext == "xml":
			...
			while pp != -1:
				...
				if len(recursos) < 1:
					...
					if color_mode: 
						cprint("\t-Descripciones encontradas en las imagenes insertadas/vinculadas: ", "cyan")
					else: 
						print "\t-Descripciones encontradas en las imagenes insertadas/vinculadas: "
...
def main (argv):
	...
	printMeta(opcion, target, color_mode)
	"""
	if len(argv) != 2: 
